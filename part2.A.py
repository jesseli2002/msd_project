...
plt.close("all")
ct.bode_plot(S, label="Sensitivity", Hz=True)
ct.bode_plot(T, label="Complementary sensitivity", Hz=True)
fig = plt.gcf()
fig.set_size_inches(8, 6)
fig.axes[0].legend(loc='lower left')
fig.axes[0].set_xlim([20, 2e4])
fig.axes[0].set_ylim(bottom=1e-8)
fig.axes[0].set_title(f"Sensitivity functions; max(S(j$\\omega$))={float(modulus_margin_db):.2f} dB @ $\\omega$ = {freq_test[np.argmax(mag)]:.1f} Hz")
fig.savefig("img2/A.2.sensitivity.png", bbox_inches="tight")
plt.close(fig)
